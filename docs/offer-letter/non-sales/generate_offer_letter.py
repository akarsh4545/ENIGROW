#!/usr/bin/env python3
"""Non-sales 2-page ENIGROW offer letter (ops, software, and similar roles).

Edit candidate.json only. Layout, clauses, and styling are frozen here.
There is no sales/revenue incentive; performance is based on role quality.
"""
...
def load_candidate(path: Path) -> dict:
    ...
    missing = [
        k
        for k in ("employee_name", "designation", "letter_date", "joining_date", "monthly_ctc")
...
def draw_border(c, doc) -> None:
    # Match vishal.pdf: hairline near the paper edge
    c.saveState()
...
def draw_page1(c, doc) -> None:
    ...
    c.setFont("TNR", 9)
    c.drawRightString(A4[0] - 8 * mm, 5.5 * mm, "Page 1 of 2")
    c.restoreState()
...
def draw_page2(c, doc) -> None:
    ...
    c.setFillColor(HexColor("#4a4a4a"))
    c.setFont("TNR", 9)
    c.drawRightString(A4[0] - 8 * mm, 5.5 * mm, "Page 2")
    c.restoreState()
...
def build_story(c: dict, S: dict) -> list:
    ...
    logo = Image(str(LOGO), width=38 * mm, height=25 * mm)
    logo.hAlign = "CENTER"
    ...
    story: list = [
        ...
        bullet(
            "The employee is expected to meet the performance standards and responsibilities associated with the "
            "assigned role, including timely completion of tasks, quality of work, technical contribution, teamwork, "
        ...
        Paragraph("4. Incentive &amp; Performance Policy", S["section"]),
        bullet(
            "The position does not carry a separate sales, revenue, or performance-based incentive structure.",
            S,
        ),
        bullet(
            "Performance may be reviewed periodically based on the quality of work, technical contribution, "
            "adherence to timelines, teamwork, and assigned responsibilities.",
            S,
        ),
        bullet(
            "The company may provide performance feedback, professional guidance, or additional responsibilities "
            "based on business requirements and individual performance.",
            S,
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a non-sales ENIGROW offer letter PDF from candidate.json")
